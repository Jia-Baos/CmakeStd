...
public:
  ...
  void dft_recursion(const T &u, set<T> &visited,
                     vector<T> &result); /* 深度优先遍历递归辅助函数 */
  ...
  void print_connected_components(
      const vector<vector<T>> &connected_components); /* 打印连通分量 */

  vector<T> articulation_points(int choice); /* 获得图中的关节点（分割点）*/

private:
  void dft(T u, T root, T parent,
           set<T> &visited_vertices, /* 获得关节点的Tarjan算法 */
           int &dfn_cnt, map<T, int> &dfn, map<T, int> &low,
           vector<T> &articulation_point_collection);
  void violent_solution(
      vector<T> &articulation_point_collection); /* 获得关节点的暴力求解法 */
};
...
template <typename T> void Graph<T>::show() {
  for (const auto &u : adj) {
    ...
    for (const auto &v : adj[u.first])
      cout << "(Adjacent Vertex: " << v.vertex
           << ", weight of edge: " << v.weight << ") ";
    cout << endl;
  }
}
...
template <typename T> int Graph<T>::num_vertices() { return adj.size(); }
...
template <typename T> Graph<T> Graph<T>::prim(T v) {
  ...
  priority_queue<pair<int, pair<T, T>>, vector<pair<int, pair<T, T>>>,
                 greater<pair<int, pair<T, T>>>>
      q;
...
template <typename T>
void Graph<T>::print_connected_components(
    const vector<vector<T>> &connected_components) {
  int number = connected_components.size();
  if (number == 1)
    cout << "The graph is a connected graph with only one connected component, "
            "which is itself."
         << endl;
  else if (number > 1) {
    cout << "Graph has" << number << "connected components" << endl;
    for (unsigned i = 0; i < connected_components.size(); i++) {
      cout << "The" << i + 1 << "nd connected component's vertexes are";
  ...
  if (choice == 1) {
    ...
    for (auto u : adj) {
      if (visited_vertices.find(u.first) == visited_vertices.end())
        dft(u.first, u.first, u.first, visited_vertices, dfn_cnt, dfn, low,
            articulation_point_collection);
    }
    int a = 1;
  }
...
template <typename T>
void Graph<T>::violent_solution(vector<T> &articulation_point_collection) {
  // 获得原来的图的连通分量数量
...
template <typename T>
void Graph<T>::dft(T u, T root, T parent, set<T> &visited_vertices,
                   int &dfn_cnt, map<T, int> &dfn, map<T, int> &low,
                   vector<T> &articulation_point_collection) {
  // 记录深度优先遍历次序
  dfn_cnt++;
  ...
  for (auto edge : adj[u]) {
    ...
    if (visited_vertices.find(v) == visited_vertices.end()) {
      ...
      dft(v, root, u, visited_vertices, dfn_cnt, dfn, low,
          articulation_point_collection);
