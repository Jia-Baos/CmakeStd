...
public:
  ThreadPool()
      : m_threads(
            std::vector<std::thread>(std::thread::hardware_concurrency())),
        m_shutdown(false) {}
  ...
  auto submit(F &&f, Args &&...args) -> std::future<decltype(f(args...))> {
    ...
    // （通过 std::async 、 std::packaged_task 或 std::promise 创建的）异步操作
    // 能提供一个 std::future 对象给该异步操作的创建者，因此 task_ptr 可以使用
    // get_future() Encapsulate it into a shared ptr in order to be able to copy
    // construct / assign
